...
st.set_page_config(layout="wide", page_title="Aegis Oracle v78.0")
st.markdown("""
    <style>
...
# --- 2. [강력 수정] 3중 지능형 검색 및 데이터 구조 완전 평탄화 (에러 해결 핵심) ---
...
def get_oracle_data(ticker, mode="일봉"):
    ...
    try:
        # [핵심 수술] auto_adjust=True를 사용하여 복잡한 OHLC 구조를 단순화합니다.
        ...
        # [필살기] 데이터프레임의 모든 컬럼 계층을 지우고 단순화 (MultiIndex 완전 제거)
        df = data.copy()
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)
        
        # 컬럼명을 표준화 (Open, High, Low, Close, Volume) - yfinance 버전에 관계없이 강제 고정
        df.columns = [str(col).capitalize() for col in df.columns]
        
        # 'Adj Close'가 없는 경우 'Close'를 복사 (auto_adjust 영향 대비)
...
if df is not None:
    ...
    with tab2:
        ...
        with cl2: st.markdown(f"""<div class="info-card"><b>🎯 실시간 타점 가이드</b><br><div style="border-left:5px solid #FF3B30; padding-left:15px; margin-top:10px;">적극 매수가: <b>{curr_p*0.98:,.0f}{unit}</b><br>목표가: <b>{curr_p*1.12:,.0f}{unit}</b><br>손절가: <b>{curr_p*0.94:,.0f}{unit}</b></div><br>AI 분석: 5,000회 시뮬레이션 결과 기대수익률 <b>{avg_sim_profit_pct:.2f}%</b> 포착.</div>""", unsafe_allow_html=True)

    with tab3: # 뉴스 (헤더 보강으로 차단 방지)
        try:
            res_n = requests.get(f"https://search.naver.com/search.naver?where=news&query={target_name} 특징주", headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'})
...
else: st.error("데이터 로드 실패: 현재 야후 파이낸스 서버 또는 종목명이 올바르지 않습니다.")
