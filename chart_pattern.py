...
st.set_page_config(layout="wide", page_title="Aegis Oracle Sovereign v105.0")
st.markdown("""
    <style>
...
# --- 2. [엔진] 하이브리드 데이터 로더 & AI 추천 엔진 ---
@st.cache_data(ttl=60)
def get_sovereign_prime_data(symbol, invest_amt):
    try:
        ...
        # 자동 알림(Alert) 로직
        alerts = []
        if last['MA5'] > last['MA20'] and prev['MA5'] <= prev['MA20']: alerts.append("🔔 골든크로스 발생 (5선 > 20선)")
        if last['RSI'] <= 30: alerts.append("⚠️ 과매도 구간 진입 (RSI 30 이하)")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("v105.0 Sovereign Prime | Stable Engine")

# --- 4. [메인] 분석 프로세스 가동 ---
df, s_name, state, alerts, exp_profit, win_rate, div_yield, fin_sum, m_type = get_sovereign_prime_data(s_symbol, invest_amt)
...
if df is not None and not df.empty:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📉 전문가 분석 차트", "🧪 정밀 퀀트 온도계", "📊 재무/실시간 뉴스 룸", "🚀 AI 엄선 초급등 테마"])
    ...
    with tab2: # 2P: 정밀 온도계
        ...
        with cl2:
            st.markdown(f'<div class="info-card"><h3>📋 AI 퀀트 레포트</h3><p>추세: {state}</p><p>AI 분석 결과, 현재 구간은 <b>{win_rate:.1f}%</b>의 확률로 반등 가능성이 있습니다.</p></div>', unsafe_allow_html=True)

    with tab3: # 3P: 재무 및 실시간 뉴스 (뉴스 풍부화)
        ...
        try:
            # 뉴스 검색 키워드를 "특징주"로 고정하여 더 많은 관련 뉴스 수집
            ...
            for item in n_soup.select('.news_area')[:10]: # 뉴스 10개 표시
                ...
                st.markdown(f"<div class='news-item'>📍 <a href='{link}' class='news-link' target='_blank'>{title}</a><br><small style='color:#aaaaaa;'>{desc}</small></div>", unsafe_allow_html=True)
        except: st.write("뉴스를 불러올 수 없습니다.")

    with tab4: # 4P: AI 엄선 초급등 테마 (AI 추천 강화)
        st.markdown("### 🚀 AI가 선정한 초급등 예상 테마 포트폴리오")
        themes = {
            "🤖 AI/반도체 (초급등 예상)": ["NVDA(엔비디아) 💎💎💎", "SK하이닉스(KR) 💎💎", "ARM(US) 💎"],
            "🛡️ K-방산/우주 (수주 랠리)": ["한화에어로스페이스(KR) 💎💎💎", "LIG넥스원(KR) 💎💎", "RTX(US) 💎"],
            "💰 금융/비트코인 (금리 변동 수혜)": ["우리금융지주(KR) 💎💎💎", "KB금융(KR) 💎💎", "COIN(US) 💎"]
        }
        cols = st.columns(3)
