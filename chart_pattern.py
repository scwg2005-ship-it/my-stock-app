...
st.set_page_config(layout="wide", page_title="Aegis Oracle v121.0")
st.markdown("""
    <style>
...
def get_final_stable_data(symbol):
    ...
    try:
        ...
        if is_kr:
            ...
            s_name = soup.select_one('title').text.split(':')[0].strip()
            
            # 재무/뉴스
            div_yield = "3.5%~ (고배당)" if "053000" in symbol else "데이터 분석중"
            fin_sum = "영업이익 및 현금흐름 우수"
...
df, s_name, win_rate, avg_profit, sims, m_type, div_yield, fin_sum, news = get_final_stable_data(s_input)
...
if df is not None and not df.empty:
    ...
    if last['MA5'] > last['MA20'] and prev['MA5'] <= prev['MA20']:
        st.markdown('<div class="alert-box">🎯 [매수 신호] 골든크로스 발생! 단기 반등이 시작되었습니다.</div>', unsafe_allow_html=True)
    ...
    st.markdown(f"""<div class="verdict-box">
        ...
        <div style="color:#cccccc;">AI 승률 <span class="highlight">{win_rate:.1f}%</span>와 수익 분포를 분석한 결과, 현재 구간은 상승 확률이 매우 높습니다.</div>
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📉 전문가 차트 분석", "🧪 지능형 온도계", "📰 재무/실시간 뉴스", "🚀 AI 엄선 테마"])
    ...
    with tab1: # 차트 가시성 강화
        ...
        clrs = {'MA5':'#FFD60A', 'MA20':'#FF37AF', 'MA60':'#00F2FF'}
        ...
        st.markdown('<div class="guide-box"><div class="guide-title">🔍 전문가 조언</div>파란 침체선에 닿으면 매수, 빨간 과열선에 닿으면 익절하십시오.</div>', unsafe_allow_html=True)

    with tab2: # 온도계+분포표 합산
        col1, col2 = st.columns(2)
        with col1:
            st.plotly_chart(go.Figure(go.Indicator(mode="gauge+number", value=win_rate, gauge={'bar':{'color':'#0055ff'}})).update_layout(template='plotly_dark', height=400))
        with col2:
            fig_h = go.Figure()
            fig_h.add_trace(go.Histogram(x=sims*100, marker_color='#007AFF', opacity=0.7))
            fig_h.update_layout(template='plotly_dark', height=400, title="수익률 분포도")
    ...
    with tab3: # 뉴스/재무
        st.markdown(f"#### 📊 재무 브리핑: {div_yield} | {fin_sum}")
    ...
    with tab4: # 테마 섹터
        ...
        themes = {"🤖 반도체/AI": ["NVDA 💎💎💎", "SK하이닉스 💎💎"], "💰 금융/저PBR": ["우리금융지주 💎💎💎", "KB금융 💎💎"]}
    ...
    st.error(f"❌ 데이터 로드 실패. (사유: {s_name})")
