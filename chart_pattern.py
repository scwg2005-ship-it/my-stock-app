...
st.set_page_config(layout="wide", page_title="Aegis Oracle v89.0")
st.markdown("""
    <style>
...
# --- 2. [에러 방어] 지능형 티커 검색 및 데이터 정형화 엔진 ---
@st.cache_data(ttl=3600)
def find_ticker_intelligent(query):
    # 주요 종목 수동 매핑 (검색 실패 차단)
    mapping = {"우리금융지주": "053000.KS", "우리금융": "053000.KS", "삼성전자": "005930.KS", "한화솔루션": "009830.KS", "SK하이닉스": "000660.KS"}
...
def get_oracle_data(ticker, mode="일봉"):
    try:
        ...
        if data.empty and ".KS" in ticker:
            data = yf.download(ticker.replace(".KS", ".KQ"), period="1y", interval="1d", progress=False)
        if data.empty: return None

        # [TypeError 방지] MultiIndex 해제 및 컬럼명 강제 표준화
        df = data.copy()
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)
        df.columns = [str(c).strip().capitalize() for c in df.columns]
        
        # [인덱스 정형화] 날짜 정보에서 시간/타임존 제거하여 충돌 방지
...
with st.sidebar:
    ...
    u_input = st.text_input("종목명(삼성전자) 또는 티커(AAPL)", value="우리금융지주")
    ...
    chart_style = st.radio("차트 형태", ["전문가 캔들", "심플 라인"], horizontal=True)
...
if df is not None:
    # 최신 가격 (에러 방지를 위해 float 변환)
    curr_p = float(df['Close'].iloc[-1])
    unit = "원" if ".KS" in ticker or ".KQ" in ticker else "$"
    ...
    st.markdown(f"### {target_name} ({ticker}) 분석")
    ...
    with c2: st.metric("현재가", f"{curr_p:,.0f}{unit}"); st.metric("AI 점수", f"{int(50 + (25 if curr_p > df['MA20'].iloc[-1] else -10))}점")
    ...
    with tab1:
        ...
        if show_rsi: fig.add_trace(go.Bar(x=df.index, y=df['Volume'], marker_color='#333', name='Vol'), row=2, col=1)
    ...
    with tab3:
        ...
        except: st.write("뉴스 일시적 로드 실패")

    with tab4:
        st.write("### 🚀 글로벌 핵심 테마 9대 카테고리")
        themes = {"🛡️ 방산/우주": ["한화에어로스페이스", "LIG넥스원"], "🤖 AI/반도체": ["NVDA", "SK하이닉스"], "💰 금융/지주": ["우리금융지주", "KB금융"]}
        cols = st.columns(3)
    ...
    st.error("데이터 로드 실패: 티커를 다시 확인하거나 잠시 후 새로고침하세요.")
